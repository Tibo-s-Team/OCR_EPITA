...
/*!
 * Give the number of file in a folder.The result is associated to a struct letter
 * This function is use to creat the list of images used for the training. 
 * @param path the path of the folder 
 * @param letter the struct letter that we want to add the number of file for a path
 * @return the characters that the neuralnetwork think it is
 */
void nbr_pictures(char* path, Letter *letter)
{
    ...
    {
        if (strcmp(ent->d_name, ".") != 0 && //the file is not the current folder
            strcmp(ent->d_name, "..") != 0) //the file is not the parent folder
        nbr++;
    }

    letter->nbr_images = nbr; //associate the number of file in the folder to the struct letter given
}

/*!
 * creat a pointer to a list of images that will be given to the training fonction. 
 * It is all the images that are in a specific folder.
 * The path of the folder is in the struct letter.
 * The result will be given to the struct letter in its .img
 * @param letter the struct letter
 * the struct must have its .path and .nbr_file completed
 */
void creat_list_img(Letter *letter)
{
    Image* list_img = malloc(letter->nbr_images * sizeof(Image)); //creat the memory space for all the images
    Image* l = list_img;
    ...
    while ((lecture = readdir(rep))) {
        ...
        //if the file is not the current folder or the father
        {
            ...
            strcat(s3,lecture->d_name); //concatenation of the path of the folder and the file name
            *l = loadImage(s3); //load the image of the s3 path file
            *l = resize_images(*l); // if the image has not the good dimension, resized it (25x25 pixels)
 ...
 * @param waited the array to print (contain all the waited value for all the letters)
...
{
    //in a first time we creat all the data that the neuralnetwork will use to learn
    //we creat a tripple pointer that is each letter ->A->B->C-> ...
    //the double poiter is each image for each letter ->A->ImgA1.png->ImgA2.png->...
    //the last pointer is for each pixels of all the images ->A->ImgA1.png->1->0->1->0->...
    double*** img_pixels = malloc(len * sizeof(double) * 625 * 331); //creat the memory space for all the data
    ...
    //thanks to the function resize_image us at the construction of struct letter
    //331 the number of images for each letters
    ...
    {
        int which_image = 0; //in order to know wich_image to creat the pointer of pixels
        double **images = malloc(sizeof(double) * 625 * 331);//init the memory space for all the images
        ...
        {
            Image *img = tab_letter[i].img + which_image; //the image to creat the pointer of pixels
            double* inputs_neuronnes = inputs(img); //creat the pointer to all the pixel of the images
    ...
    {
        int which_letter = 0;
        for (double*** i = img_pixels; i < img_pixels+len; i++)//for each letters
        {
            for (double** j = *i; j < *i + tab_letter[which_letter].nbr_images; j++)//for each images
            {   
                one_training(neuralnetwork, *j, 625, waited[which_letter], coeff); //do one trainning 
            }
            which_letter+=1;
        }
...
Image resize_images(Image img)
{
    if(img.height < 25 || img.width < 25)// if the image is too small
    {
        ...
        for(int i = 0; i < img.width; i++)
        {
            for(int j = 0; j < img.height; j++)
            {
                ...
                Uint8 g = 0;
                Uint8 b = 0;
                getPixelRGB(&img, i, j, &r, &g, &b); //get the pixel(i, j) of the too small image
                Uint8 color[3] = {r, g, b}; //creat the color of this pixel
                setPixelColor(&blanc, *color, decallage_x+i, decallage_y+j); //put the pixel in the white image
            }
        }
        return blanc;
    }
    else if (img.height >= 26 || img.width >= 26)
    {
        ...
        //this part of the function is juste to avoid a crash of our neurlanetwork
